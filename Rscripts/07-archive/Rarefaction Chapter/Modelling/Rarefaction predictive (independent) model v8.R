...
# based on outputs from Rarefaction [independent] model v8
...
# data ----------------------------------------------------------------   
# model data
  load("Rarefaction/Rdata/Independent_rarefaction_model_coefficient_values.RData")
...
# 1 predicting distributions ------------------------------------------------
# native ----------------------------------------------------------------------
# note: can't do total atm because I haven't got that in the previous script -- will do later (but it won't change)
# Total richness
...
# bind predicted distributions and add cell id
  pred.distr <- cbind(cell.id, 
                      nat.c3.pred, nat.c4.pred, # add total in here later (see v7 for exact methods)
                      exo.c3.pred, exo.c4.pred) # exotic totla here , too
...
# make a matrix with all missing values
  x <- matrix(NA, nrow = length(getValues(aus)), ncol = 4) # 6
  
  pred.distr.m <- as.matrix(pred.distr)
  pred.distr.mm <- pred.distr.m[, 2:5]
...
# Native  
  ...
  writeRaster(n.tot.p, "n.tot.predicted", overwrite = T) # check numbers when doing total later
  plot(n.tot.p)
  
  n.c3.p <- setValues(aus, x[, 1])
  writeRaster(n.c3.p, "n.c3.predicted", overwrite = T)
  plot(n.c3.p)
  
  n.c4.p <- setValues(aus, x[, 2])
...
# Exotic  
  ...
  e.c3.p <- setValues(aus, x[, 3])
  writeRaster(e.c3.p, "e.c3.predicted", overwrite = T)
  plot(e.c3.p)
  
  e.c4.p <- setValues(aus, x[, 4])
