...
# number of species per cell
  poa <- as.numeric(factor(spp$species))
  n_spp <- rasterize(xy, raster, field = poa, fun = function(x,...) {length(unique(na.omit(x))) })
  plot(n_spp)
...
# select the columns we need
  sp <- spp %>%
    dplyr::select(species, pp, year, cell)
...
# loop through all the cells, extract data from dataframe sp, and calculate rarefied richness using n.min as the number of samples for rarefaction
  ...
  ratio.ne <- setValues(raster, x[, 2]/x[, 1])
  plot(ratio.ne)
  ...
  n_spp_nat <- rasterize(new.xy, raster, field = spp, fun = function(x,...) {length(unique(na.omit(x))) })
  ...
  n_spp_int <- rasterize(new.xy, raster, field = spp, fun = function(x,...) {length(unique(na.omit(x))) })
