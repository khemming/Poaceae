...
# -----------------------------------------------------------------------------------
# 1. oraganise APC native/exotic [naturlaised] tags -------------------------------------
...
# -----------------------------------------------------------------------------------
# 2. create distribution (state) fields that AVh data can handle --------------------
...
# notes --------------------------------------------------------------------------
# removing: hybrids, records missing required column data (e.g. species name, lat, long, genus, year), [one] subspecies, and duplicate records
# ---------------------------------------------------------------------------------
# read in RDS file (note this was downloaded as CSV and I converted it to save space)
  avh.raw <- readRDS("AVH/Supplied data/records-2019-03-01.rds")
  
# required columns listed in header csv
  avh.header <- read.csv("AVH/Supplied data/records-2019-03-01 headings.csv", header = T) %>%
                select(Required)
  avh.header <- as.vector(t(avh.header))
  avh.header.pos <- which(avh.header == "yes")
  
# select columns  
  avh <- avh.raw[, ..avh.header.pos]
  avh <- data.frame(avh)
  
# species ranked records
  avh1 <- filter(avh, taxonRank == "species")

# remove subspecies
  avh2 <- filter(avh1, !grepl("subsp. ", species))

# remove incorrect year (where year = 0)
  avh3 <- filter(avh2, !grepl("0", year))
  
# remove rows without one of: species, genus, lat, long, year
  avh4 <- avh3 %>% tidyr::drop_na("species", "genus", "decimalLatitude", "decimalLongitude", "year")
  
# remove duplicates
# round lat/longs to ~1-km (2dp)
  avh4$decimalLatitude <- round(avh4$decimalLatitude, digits = 2)
  avh4$decimalLongitude <- round(avh4$decimalLongitude, digits = 2)
# find unique (distinct) records 
  avh5 <- avh4 %>% distinct(species, year, decimalLatitude, decimalLongitude, .keep_all = TRUE) 
# removes a boatload of records

# list of species
  spp.list <- levels(as.factor(avh5$species))
  length(spp.list)
  spp.list.df <- data.frame(spp.list) # 1649 -- many!

  saveRDS(avh5, "AVH/Generated data from cleaning steps/ALA cleaned Poaceae.rds")
  
# -----------------------------------------------------------------------------------------
# 4. filter ALA spp. names with APC accepted names -----------------------------------------  
# ALA data thus far
  rm(list = ls())
  avh <- readRDS("AVH/Generated data from cleaning steps/ALA cleaned Poaceae.rds")
...
# create data frame of all avh species names
  avh$species <- as.factor(avh$species)
  avh.names <- data.frame(sci.name = as.character(levels(avh$species)), avh = 1) 
...
# take only names which have both AVH and APC (i.e. tot = 2)  
  matching.names <- filter(allnames, tot == 2)
  
# subset avh based on these names
  avh$species <- as.character(avh$species)
  apc.names$sci.name <- as.character(apc.names$sci.name)
  avh.apc.names <- filter(avh, avh$species %in% matching.names$sci.name)
  
# add native status to avh data
  colnames(apc.names) <- c("species", "apc", "native")
  avh.status <- left_join(avh.apc.names, apc.names, by = "species")
  
  saveRDS(avh.status, "AVH/Generated data from cleaning steps/ALA cleaned APC Poaceae.rds")
  
# ----------------------------------------------------------------------------------------
# 5. doublecheck APC tags with Rachel/Randall's list ----------------------------
  rm(list = ls())
# read in cleaned renamed data 
  kh <- readRDS("AVH/Generated data from cleaning steps/ALA cleaned APC Poaceae.rds")
...
# correct column names/remove ones not needed
  kh3 <- kh.rg.spp.2 %>% dplyr::select(-native.rg, -apc, -id) %>%
          mutate(native = native.kh) %>%
          select(species, native, -native.kh)
  spp.list$species <- as.character(spp.list$species)
 
# add native status tag to data
  kh.7 <- select(kh, -native, -apc)
  kh8 <- left_join(kh.7, kh3, by = "species")

# save 
  saveRDS(kh8, "AVH/Generated data from cleaning steps/ALA cleaned APC-RG Poaceae.rds")
...
# ALA data
  rm(list = ls())
  ala <- readRDS("ALA/Generated data from cleaning steps/ALA cleaned APC-RG Poaceae.rds") %>%
...
# Calamagrostis --> C3; "             "
# Desmostachya --> C4; Chloridoideae (1)
# Milium --> C3 (exo)
# Schmidtia -- C4 (exo)
