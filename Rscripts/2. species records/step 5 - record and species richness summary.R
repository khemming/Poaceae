...
# records, raw species richness --------------------------------------------
  spp_no <- dat %>% 
            ...
            distinct(species) %>%
            summarise(sr = n()) %>%
            pivot_wider(names_from = pp, values_from = sr) %>%
            mutate(spp_tot = C3 + C4,
                   pp_ratio = C4/C3)
  spp_no$pp_ratio <- round(spp_no$pp_ratio, 2)
...
# merge
  table <- left_join(spp_no, rec_no, by = "status") %>%
           adorn_totals() %>%
    dplyr::select(status, spp_tot, C3, C4, pp_ratio, recs)
