...
#           raster.name -- identity for file location, so maybe put in if its observed or predicted, and in quotes
  prop_sr <- function(raster, raster.name) {
    ...
    rasterfile <- paste0("Results/rasters/scaled/", raster.name, ".grd")
...
# native
  prop_sr(native_C3, "native_C3")
  prop_sr(native_C4, "native_C4")
  prop_sr(native_total, "native_total")
# non-native (exotic)
  prop_sr(nonnative_C3, "nonnative_C3")
  prop_sr(nonnative_C4, "nonnative_C4")
  prop_sr(nonnative_total, "nonnative_total")
