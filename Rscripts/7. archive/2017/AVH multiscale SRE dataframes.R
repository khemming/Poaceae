...
# AVH EDA 
# -- What is the minimum scale I can plot EF-spp at?
...
# exclude cells outside of Australia
  ichao1_na <- data.frame(b, a, n, f1, f2, f3, f4)
  ichao1 <- ichao1_na[!is.na(b), ] 
    
# Jackknife estimators ----------------------------------------
# from Chapter 4., we'll do both first- and second-order estimates 
...
# f2 bias-corrected form included
    lanum_f2 <- function(x) {
      ichao1$a + (3*(ichao1$f1^3)*ichao1$f3)/(4*(ichao1$f2^3))
    } 
    
    lanum_f1 <- function(x) {
      ichao1$a + (3/4 * (ichao1$f1*(ichao1$f1-1)*(ichao1$f1-2)*ichao1$f3) / ((ichao1$f2 + 1) * (ichao1$f2 + 2) * (ichao1$f2 + 3)))
    } 
   
    ichao1$lanum <- ifelse(ichao1$f2 == 0, lanum_f1(ichao1), lanum_f2(ichao1)) 
...
# Pearson's correlation coefficients between record# and SRE (6)
  n_a_cor <- cor(ichao1$n, ichao1$a, method = "pearson") 
  n_ichao1_cor <- cor(ichao1$n, ichao1$ichao1, method = "pearson")
  n_chao1_cor <- cor(ichao1$n, ichao1$chao1, method = "pearson")
  n_jack1st_cor <- cor(ichao1$n, ichao1$jack1st, method = "pearson")
  n_jack2nd_cor <- cor(ichao1$n, ichao1$jack2nd, method = "pearson")
  n_lanum_cor <- cor(ichao1$n, ichao1$lanum, method = "pearson")
...
# scale range (2 - 500 km)
  ...
  colnames(ichao_avh) <- c("cell_width", "n", "a", "f1", "f2", "f3", "f4",
                           # 6 + 1
                           "ichao1_sre", "chao1_sre", "jack1st", "jack2nd", "lanum",
                           # 5
                           "chiu_se", "upper_ci", "lower_ci",
...
# csv 
  write.csv(ichao_avh, "C:/Users/s436862/Dropbox/Climate Matching/4. Results/AVH/EDA/multiscale EDA AVH.csv", row.names = F)
  # note: still need to add 1km scale in there 
