...
# checking what cutoff is sueful
  int_25 <- raster("C:/Users/s436862/Dropbox/Rarefaction/4. Results/Rarefaction/Rasters/Int_rare_25")
  plot(int_25) # was this calculated in the presence of native? Me thinks so. Therefore, not useful. Let's start again. Cool. New script? Hmm. Maybe..
...
# Keep only this bit (because otherwise a gives me weird SR values)
  ...
  write.csv(rarefaction, file = "C:/Users/s436862/Dropbox/Rarefaction/4. Results/Rarefaction/CSV/Nat_ONLY rarefaction.csv", row.names = F)
# ----------------------------------------------------------------------------
  

# Rarefaction plots for native and introduced rarefied richness (and just native richness, I guess) -----------------------------------------
...
# function inputs    
# Native 
  nat <- raster("4. Results/Rarefaction/Rasters/Nat_rare_50")
  plot(nat)
  legend <- "Native richness (rarefied)"
  save <- "4. Results/Rarefaction/Graphs/Native_rarefied.jpeg"
  
  eng_ras(nat, legend, save)
  
 # introdcued
  int <- raster("4. Results/Rarefaction/Rasters/Int_rare_50")
  legend <- "Introduced richness (rarefied)"
  plot(int)
  save <- "4. Results/Rarefaction/Graphs/Introduced_rarefied.jpeg"
  
  eng_ras(int, legend, save)
...
# Data ---------------------------------------------------------------------------
# Bring the species and environmental data together
  
# native and introduced rarefaction
  spp <- read.csv("C:/Users/s436862/Dropbox/Rarefaction/4. Results/Rarefaction/CSV/Rarefied richness nat int C3-4.csv", header = T) %>%
    select(-X)
...
# Refer to notes in Rarefaction document v.5.0.1 for the specifics  
  ef <- read.csv("1. Data files/EFs/3. EFs aggregated/EF_COMPLETE.csv", header = T) %>%
...
# Bind & use only Aus cells
  cell_category <- read.csv("1. Data files/EFs/3. EFs aggregated/EF_COMPLETE.csv", header = T) %>%
...
# 3.1 Modelling ---------------------------------------------------------------------  
# I will model and test VIF scores simultaneously
# Section will be broken into:
...
# Native models -------------------------------------------------------------------  
# Firstly, the VIF score 
  nat_vif <- vif(lm(nat_total ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef))
  nat_vif  # Beautiful
  
# M1: C3
  m1 <- lm(nat_c3 ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m1) # 0.88, v. cool
  
# M2: C4
  m2 <- lm(nat_c4 ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m2) # 0.77, HII not significant, interesting
  
# M3: total
  m3 <- lm(nat_total ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m3) # 0.36, v. interesting
  
# Exotic models -------------------------------------------------------------------
# Firstly, the VIF score 
  int_vif <- vif(lm(int_total ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef))
  int_vif  # Beautiful
 
# M4: C3
  m4 <- lm(int_c3 ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m4) # 0.82, v. cool
  
# M5: C4
  m5 <- lm(int_c4 ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m5) # 0.68
  
# M6: total
  m6 <- lm(int_total ~ ap + pcoldq + twarmm + tcoldm + pewc + th + hii, data = spp_ef)
  summary(m6) # 0.51
