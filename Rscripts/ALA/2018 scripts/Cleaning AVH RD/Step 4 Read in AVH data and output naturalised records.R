...
# extract names from avh and apc and compare
  ...
  apc.names <- data.frame(sci.name = as.character(levels(apc.nat$name)), apc = 1, naturalised = table(apc.nat$name, apc.nat$naturalised)[, 2])
...
# list of naturalised species names that are in avh data
  nat_names <- allnames$sci.name[allnames$avh == 1 & (is.na(allnames$naturalised) == F & allnames$naturalised == 1)]

# fix up avh names with " x "
  avh$Species...matched <- gsub(" x ", " ", avh$Species...matched)
  avh$naturalised <- ifelse(avh$Species...matched %in% nat_names, 1, 0)
  
  avh.nat <- filter(avh, naturalised == 1)
  dim(avh.nat)
  table(avh.nat$Taxon.identification.issue, avh.nat$Name.not.in.national.checklists)
  
# output all avh naturalised records
  write.csv(avh.nat, "c:\\users\\s429217\\onedrive\\data\\avh\\AVH naturalised records.csv")
