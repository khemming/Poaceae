...
# 3.7 Introduced Chao1 plot ------------------------------------------
# May or may not use this for the conference  
  int.c.c <- raster("C:/Users/s436862/Dropbox/Rarefaction/Results/SRE methods/Rasters/Int_chao1")
  plot(int.c.c)
  
  aus.shp <- readOGR("C:/Users/s436862/Dropbox/Climate Matching/Data files/Australia/australia_shapefile.shp")
  
  int.c <- mask(int.c.c, aus.shp)
  plot(int.c)
  
# Run plot function
  raster <- int.c
  legend <- "Species richness"
  save <- "C:/Users/s436862/Dropbox/Rarefaction/Results/Rarefaction/Graphs/Int chao1.jpeg"
...
# 3. Native-exotic correlation scatterplots ------------------------------------------
# Here I think looking at nat vs. Int species richness in a few key areas will be good

# Data -----------------------------------------------------------------
# Spp data
  spp <- read.csv("CSV/Rarefied_30 Nat_Int Total_C3_4.csv", header = T)
  
# Environmental factors  
  ef <- read.csv("C:/Users/s436862/Dropbox/Climate Matching/Data files/EFs/EFs aggregated/EF_COMPLETE.csv", header = T) %>%
    select(ap, pcoldq, twarmm, tcoldm, pewc, th, hii) 
  
# standardise EFs  
  ef.s <- data.frame(scale(ef, center = T, scale = T)) 
  
# Bind & use only Aus cells
  cell_category <- read.csv("C:/Users/s436862/Dropbox/Climate Matching/Data files/EFs/EFs aggregated/EF_COMPLETE.csv", header = T) %>%
    select(cell_category)
  
  spp_ef <- cbind(cell_category, spp, ef.s) %>%
    filter(cell_category == "terrestrial") %>%
    select(-cell_category) # al dente
...
# Nat-C3 vs AP
  ...
  save <- "Results/Rarefaction/Graphs/Single variate scatter plots/Nat C3-AP.jpeg" 
...
# Nat-Int coefs in same whisker plot ---------------------------  
# # Andrew's original code: 
#   c3_coef <- spp.ef
#   c4_coef <- mutate(c4_coef, photosystem = "C4")
#   
#   coef_all <- bind_rows(c3_coef, c4_coef)
#   
#   # You might need to tune the positions
#   ggplot(coef_all, aes(x = reorder(coef, desc(mean)), color = origin)) +
#     
#     geom_hline(aes(yintercept = 1),
#                color = "red", size = 0.6) +
#     geom_point(aes(y = mean), 
#                size = 3, position = position_dodge(width = 1)) +
#     geom_errorbar(aes(ymin = conf_low, = conf_high),
#                   size = 1, width = 0, position = position_dodge(width = 1)) +
#     annotate("segment", x = -Inf, xend = Inf, = -Inf, yend = -Inf) +
#     annotate("segment", x = -Inf, xend = -Inf,= -Inf, yend = Inf) +
#     facet_grid( ~ photosystem)
#   
  
# Remove x axis: Margarita's code -------------------------------------------
#plot that includes the predetermined colours
#yintercept is to specify where you want the cutoff and you can also change the line colour
  
  ggplot(Path4h, aes(x = reorder(Term, log10P), y = log10P, fill=Name)) +
    geom_bar(aes(colour = Name, group = Name), col= "black", stat = "identity", position = "dodge") +
    coord_flip() +
    labs(title="4hrs FDM", y="-log10(P)", x="")+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background= element_blank(), axis.line = element_line (colour = "black"), legend.position = "bottom")+ geom_hline(yintercept = 3, col="black", linetype="dashed")
  
# ---------------------------------------------------------------  

# C3-C4 ----------------------------------------
# Data frame with all photosynthetic classes in it
  levels(c3.rich$coef) <- c("PCOLDQ", "PWARMQ", "AMT", "PS", "ARID", "PEWC", "TH", "HII")
   
  levels(c4.rich$coef) <- c("PCOLDQ", "PWARMQ", "AMT", "PS", "ARID", "PEWC", "TH", "HII")
  
  levels(total.rich$coef) <- c("PCOLDQ", "PWARMQ", "AMT", "PS", "ARID", "PEWC", "TH", "HII")
 
# -------------------------------------------
# Both C3 and C4 in same plot (not used) -------------------------------
  coef.all <- rbind(c3.coef, c4.coef) %>%
    mutate(coef = factor(coef, levels = c("pcoldq", "pwarmq", "mat", "ps", "arid", "pewc", "th", "hii")))
  
  ggplot(coef.all, aes(x = coef, color = status)) +
    theme_classic() +
  ...
  ggsave("Results/Rarefaction/Graphs/Coefficients/Combined C3-C4 coefficients.jpeg", plot = last_plot(), scale = 1, dpi = 500, device = "jpeg")
...
# Total plots ----------------------------------------------  
# Native  
  raster <- raster("Rasters/Predicted distributions/n.tot.predicted.grd")/17.29820 # +0.00001 (because of non-plotting issues)
  legend <- "Species richness"
...
# Exotic 
  raster <- raster("Rasters/Predicted distributions/e.tot.predicted.grd")/17.29820 # +0.00001 (because of non-plotting issues)
  legend <- "Species richness"
...
# C3 plots ----------------------------------------------
# Legend scale adjusted due to predicted values outside of 0-1 range
# Adjusted legend function --------------------------------------------  
  eng_ras_c3_pred <- function (raster, legend, save)  
    
  {
    ...
    q <- gplot(raster) + 
      ...
      scale_fill_gradientn(colours = colour, 
                           space = "Lab",
                           limits = c(-0.3793215, 1), # +/- 0.0000001 for plotting
                           na.value = "transparent",
                           guide = "colourbar",
                           name = legend) + 
      coord_equal() +
  ...
  # Note: for standardising, change legend limits to: -0.3793214, 0.9999997  (standardised min, max)
  
# Native  
  raster <- raster("Rasters/Predicted distributions/n.c3.predicted.grd")/13.61201 # absolute maximum of native/exotic C3
  legend <- "Species richness"
  save <- "Graphs/Predicted distributions/Native C3.jpeg"
  
  eng_ras_c3_pred(raster, legend, save)
  
# Exotic C3 plot 
  raster <- raster("Rasters/Predicted distributions/e.c3.predicted.grd")/13.61201
  legend <- "Species richness"
...
# Legend scale adjusted due to predicted values outside of 0-1 range
# Adjusted legend function --------------------------------------------  
  eng_ras_c4_pred <- function (raster, legend, save)  
    
  {
    ...
    q <- gplot(raster) + 
      ...
      scale_fill_gradientn(colours = colour, 
                           space = "Lab",
                           limits = c(-0.09128906, 1), # +/- 0.0000001 for plotting
                           na.value = "transparent",
                           guide = "colourbar",
                           name = legend) + 
      coord_equal() +
  ...
  # Note: for standardising, change legend limits to: -0.09128905, 1  (standardised min, max)
  
# Native
  raster <- raster("Rasters/Predicted distributions/n.c4.predicted.grd")/19.60622
  legend <- "Species richness"
  save <- "Graphs/Predicted distributions/Native C4.jpeg"
  
  eng_ras_c4_pred(raster, legend, save)
  
# Exotic ----------------------------------------------  
  raster <- raster("Rasters/Predicted distributions/e.c4.predicted.grd")/19.60622
  legend <- "Species richness"
