...
# Load model workspace (from Rarefaction chapter model v5)
  rm(list = ls())
  setwd("C:/Users/s436862/Dropbox/Rarefaction")
  load("Results/Rarefaction/Rdata/rarefaction_model_coefficient_values.RData")
...
# C3 -------------------------------------------------------------------
# c3 adjusted r2 & position: x = 7, y = 1, label = "Adj. r2 = exotic 0.82, native 0.89"
  i <- ggplot(c3.rich, aes(x = plot.names, color = status)) +
    ...
    annotate("segment", x = -Inf, xend = Inf, y = -Inf, yend = -Inf) +
    annotate("segment", x = -Inf, xend = -Inf,y = -Inf, yend = Inf) + 
    annotate("text", x = 7, y = 1.1, label = "Adj. R2 = exotic 0.82, native 0.89")
